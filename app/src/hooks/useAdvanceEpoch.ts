...
export function useAdvanceEpoch() {
  ...
  const advanceEpoch = useCallback(
    async (vaultId: number): Promise<string | null> => {
      ...
      const toastId = addToast({
        variant: 'pending',
        title: 'Advancing epoch...',
        description: 'Waiting for wallet approval',
      });

      try {
        const tx = await client.advanceEpoch(vaultId);
        ...
        updateToast(toastId, {
          ...
          description: 'New epoch has started',
